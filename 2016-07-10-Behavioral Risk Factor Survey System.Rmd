...
Load the needed packages and load and tidy data.
...
#### Mental Health by State per Month the Interview Took Place
...
There seems to be no clear patterns on mental health and time of the year, even when we take into account the different locations. It maybe looks that the more south we go, people are having more not good days. We would need to perform statistical tests to get more certainty.


## 2. How different sport activities correlate with mental and physical health?

It is interesting to see are practitioners of some activities doing better physically and mentally. Here we have to be especially careful of not mixing causality and correlation. Many confounding variables are possible. Subject's age comes to mind first as an example. People tend to have different hobbies in different ages and could have different types of problems depending on age.
...
First check is to see if there are differences between exercising and non-exercising groups via calculating mean number of days for exercising and non-exercising groups.
...
Nothing shocking there. People who exercise have noticeably less physical and mental bad days. Here is a good place to think the chicken-or-egg causality dilemma though.
...
Upper Body Cycle, household activities and fishing are the ones having most physical problems. Skateboarding and childcare seem to be worst mentally. People who do racquetball, tennis or golf seem to be doing the best.

## 3. Is there a relationship between economic wellbeing and alcohol consumption?

This is socially and politically an important question. Alcohol consumption has been proven to affect many health conditions adversely. Let's see if income class is related to this problem.
...
# few subjects drinking more than 10 per day are removed to make the plot more readable
...
# people drinking more than 10 per day are removed to make the plot more readable
...
The relationship between income class and alcohol consumption is completely opposite now. It seems that there is a link between income class and alcohol use. Based on exploratory analysis it looks like subjects who drink, are drinking more the less they earn. It would be interesting to do more graphs with different parameters. For example, decide a number of daily drinks that starts to be problematic and look at the patterns then.
